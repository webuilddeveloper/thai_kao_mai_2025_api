...
namespace thai_kao_mai_api.Controllers.Mobile
{
    [Route("m/[controller]")]
    public class PolicyPartyController : Controller
    {
        ...
        public ActionResult<Response> readMain([FromBody] Criteria value)
        {
            try
            {
                ...
                var filter = Builders<PolicyParty>.Filter.Eq("status", "A");

                if (!string.IsNullOrEmpty(value.code)) { filter = filter & Builders<PolicyParty>.Filter.Regex("code", value.code); }

                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = filter & Builders<PolicyParty>.Filter.Regex("title", value.keySearch);
                ...
                if (!string.IsNullOrEmpty(value.category)) { filter = filter & Builders<PolicyParty>.Filter.Eq("category", value.category); }

                if (!string.IsNullOrEmpty(value.language)) { filter = filter & Builders<PolicyParty>.Filter.Regex("language", value.language); }
                if (value.isHighlight) { filter = filter & Builders<PolicyParty>.Filter.Eq("isHighlight", value.isHighlight); }
                ...
                List<PolicyParty> docs = col.Aggregate().Match(filter).SortBy(o => o.sequence).ThenByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit)
                ...
                try
                {
                    ...
                    valueCategory.statisticsCreateAsync("policyPartyCategory");
                }
                catch { }
                //END : Statistic
                ...
                if (!string.IsNullOrEmpty(value.code))
                {
                    ...
                    var colUpdate = new Database().MongoClient("policyParty");
        ...
        public ActionResult<Response> GalleryRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Gallery>("policyPartyGallery");
        ...
        public ActionResult<Response> GalleryFileRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Gallery>("policyPartyGalleryFile");
        ...
        public ActionResult<Response> Count([FromBody] Criteria value)
        {
            ...
            {
                var col = new Database().MongoClient<PolicyParty>("policyParty");
                var filter = (Builders<PolicyParty>.Filter.Eq("status", "A") & value.filterOrganization<PolicyParty>());
