    ...
    public class SuggestionController : Controller
    {
        ...
        public ActionResult<Response> readMain([FromBody] Criteria value)
        {
            try
            {
                ...
                var filter = Builders<Suggestion>.Filter.Eq("status", "A");

                if (!string.IsNullOrEmpty(value.code)) { filter = filter & Builders<Suggestion>.Filter.Regex("code", value.code); }
           
                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = filter & Builders<Suggestion>.Filter.Regex("title", value.keySearch);
                    ...
                    try
                    {
                        ...
                        value1.statisticsCreateAsync("suggestionKeySearch");
                    }
                    catch { }
                    //END : Statistic
                }
                if (!string.IsNullOrEmpty(value.category)) { filter = filter & Builders<Suggestion>.Filter.Eq("category", value.category); }
                if (!string.IsNullOrEmpty(value.productCode)) { filter = filter & Builders<Suggestion>.Filter.Eq("productCode", value.productCode); }

                if (!string.IsNullOrEmpty(value.language)) { filter = filter & Builders<Suggestion>.Filter.Regex("language", value.language); }
                if (value.isHighlight) { filter = filter & Builders<Suggestion>.Filter.Eq("isHighlight", value.isHighlight); }
                ...
                try
                {
                    ...
                    valueCategory.statisticsCreateAsync("suggestionCategory");
                }
                catch { }
                //END : Statistic
                ...
                if (!string.IsNullOrEmpty(value.code))
                {
                    ...
                    var colUpdate = new Database().MongoClient("suggestion");
        ...
        public ActionResult<Response> GalleryFileRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Gallery>("suggestionGalleryFile");
        ...
        public ActionResult<Response> CommentRead([FromBody] Criteria value)
        {
            try
            {
                ...
                var col = new Database().MongoClient<Comment>("suggestionComment");
        ...
        public ActionResult<Response> CommentCreate([FromBody] Comment value)
        {
            ...
            try
            {
                ...
                var col = new Database().MongoClient("suggestionComment");
        ...
        public ActionResult<Response> Count([FromBody] Criteria value)
        {
            ...
            {
                var col = new Database().MongoClient<Suggestion>("suggestion");
                var filter = (Builders<Suggestion>.Filter.Eq("status", "A") & value.filterOrganization<Suggestion>());
