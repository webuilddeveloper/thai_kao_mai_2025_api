    ...
    public class DonateController : Controller
    {
        ...
        public ActionResult<Response> readMain([FromBody] Criteria value)
        {
            try
            {
                ...
                var filter = Builders<Donate>.Filter.Eq("status", "A");

                if (!string.IsNullOrEmpty(value.code)) { filter = filter & Builders<Donate>.Filter.Regex("code", value.code); }

                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = filter & Builders<Donate>.Filter.Regex("title", value.keySearch);
                    ...
                    try
                    {
                        ...
                        value1.statisticsCreateAsync("DonateKeySearch");
                    }
                    catch { }
                    //END : Statistic
                }
                if (!string.IsNullOrEmpty(value.category)) { filter = filter & Builders<Donate>.Filter.Eq("category", value.category); }

                if (!string.IsNullOrEmpty(value.language)) { filter = filter & Builders<Donate>.Filter.Regex("language", value.language); }
                if (value.isHighlight) { filter = filter & Builders<Donate>.Filter.Eq("isHighlight", value.isHighlight); }
                ...
                try
                {
                    ...
                    valueCategory.statisticsCreateAsync("donateCategory");
                }
                catch { }
                //END : Statistic
                ...
                if (!string.IsNullOrEmpty(value.code))
                {
                    ...
                    var colUpdate = new Database().MongoClient("donate");
        ...
        public ActionResult<Response> GalleryRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Gallery>("donateGallery");
        ...
        public ActionResult<Response> GalleryFileRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Gallery>("donateGalleryFile");
        ...
        public ActionResult<Response> Count([FromBody] Criteria value)
        {
            ...
            {
                var col = new Database().MongoClient<Donate>("donate");
                var filter = (Builders<Donate>.Filter.Eq("status", "A") & value.filterOrganization<Donate>());
